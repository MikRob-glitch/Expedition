...
**Carte du repérage (#73)** — au-dessus de la liste, pour savoir *où aller maintenant* et pas seulement *ce qu'il reste à faire* : un marqueur numéroté par indice localisé (vert dès que sa photo est prise), la **position en direct** avec son **halo de précision**, la **distance** sur chaque fiche, **➜ Itinéraire** vers l'application de cartes du téléphone en mode vélo, et **🗺 Sur la carte** / clic sur un marqueur qui cadre le point *et* fait remonter sa fiche. La carte se replie.

Le bouton **« Me suivre »** recentre, et **se coupe dès qu'on fait glisser la carte** : faire glisser, c'est vouloir regarder ailleurs.

⚠️ **`watchPosition` est arrêté en quittant l'écran** (`destroyReconMap`) : sinon le GPS tourne en arrière-plan et des callbacks écrivent dans un DOM détruit.
⚠️ **Les distances s'écrivent dans le DOM sans repeindre la liste** (`paintReconDistances`) — repeindre perdrait la saisie en cours, même raison que `paintClueGeo` (règle #49).
⚠️ **`geo:` n'ouvre rien sur iOS** : l'itinéraire passe par une URL Google Maps `dir`.
...
| `initReconMap` / `syncReconMarkers` / `destroyReconMap` | Carte du repérage : marqueurs des indices localisés, destruction **avec arrêt du GPS** |
| `startReconWatch` / `stopReconWatch` / `paintMe` | Position en direct + halo de précision ; `watchPosition` toujours refermé |
| `toggleFollow` / `paintFollowBtn` | Recentrage automatique, coupé dès que l'on fait glisser la carte |
| `haversine` / `fmtDist` / `paintReconDistances` | Distance à chaque indice, écrite dans le DOM sans repeindre la liste |
| `focusReconClue` / `reconRoute` | Cadrer un point et remonter sa fiche · itinéraire vélo dans l'app de cartes |
...
17. **L'indice photo, le repérage et sa carte n'ont jamais tourné dans un vrai navigateur** (#72/#73), ni sur un événement — ni le rendu Leaflet, ni le GPS réel, ni le comportement du suivi en marchant. Le banc ne voit ni la caméra, ni le GPS réel, ni les RLS, ni le Storage : la prise de photo depuis un téléphone est le seul geste du lot dont personne n'a encore vu le résultat. ⚠️ `migration-hints.sql` doit être jouée — sans elle il n'y a simplement pas d'indice photo, l'application et la console continuent de fonctionner.
...
node tests/test-hints.js    # 91 — indice photo payant (le prix fait foi côté games) + repérage + carte
