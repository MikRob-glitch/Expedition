...
- **Indice photo payant** — une équipe coincée peut acheter la **photo du lieu**, prise au repérage, contre des points (**30 par défaut**, réglable indice par indice). L'achat est définitif, il apparaît dans la console, et le score peut passer sous zéro — refuser l'achat bloquerait l'équipe au moment précis où elle est coincée. Le prix vit du côté de la chasse, que seul le maître du jeu peut écrire : **une équipe ne peut pas se fabriquer un tarif**.
- **Repérage terrain** — un mode mobile de la console pour préparer une chasse **sur place**, indice par indice : la photo indice, et surtout la **position réelle relevée au GPS** avec sa précision affichée. Une **carte** montre les points et **votre position en direct** avec sa précision, la distance à chaque indice, et un itinéraire vélo vers celui que vous visez. Un QR fait passer le repérage du poste fixe au téléphone. C'est ce qui transforme un parcours écrit au bureau (coordonnées à ±100 m, invalidables) en parcours vendable.
...
>
> ⚠️ Les bancs sont hors ligne et se rejouent en une commande : `npm i jsdom` puis `node tests/test-print.js` (38 tests — géométrie du cadre, dont l'invariant « rien de décoratif à moins de 4 mm du bord »), `node tests/test-map.js` (50 — carte live, minimap, position), `node tests/test-merge.js` (29 — fusion d'équipes) `node tests/test-templates.js` (123 — chasses types : normalisation, import par fichier, carte de l'éditeur, garde-fous d'enregistrement) et `node tests/test-hints.js` (91 — indice photo payant, repérage et sa carte) — **331 au total**. **Conserver tout nouveau banc dans `tests/`**, jamais dans `/tmp` : c'est pour l'avoir oublié qu'un cadre imprimé hors du papier n'a été vu qu'au tirage.
>
