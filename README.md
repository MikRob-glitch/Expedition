...
>
> ⚠️ Les bancs sont hors ligne et se rejouent en une commande : `npm i jsdom` puis `node tests/test-print.js` (38 tests — géométrie du cadre, dont l'invariant « rien de décoratif à moins de 4 mm du bord »), `node tests/test-map.js` (50 — carte live, minimap, position), `node tests/test-merge.js` (29 — fusion d'équipes) et `node tests/test-templates.js` (123 — chasses types : normalisation, import par fichier, carte de l'éditeur, garde-fous d'enregistrement) — **240 au total**. **Conserver tout nouveau banc dans `tests/`**, jamais dans `/tmp` : c'est pour l'avoir oublié qu'un cadre imprimé hors du papier n'a été vu qu'au tirage.
>
